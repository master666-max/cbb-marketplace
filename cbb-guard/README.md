...
## 回归夹具（v0.1.1 起随修复一同跑，三套共 41 例）

| 套 | 覆盖面 | 例数 | v0.1.0 基线 | v0.1.1 |
|---|---|---|---|---|
| 主夹具 | 真危险 13 例（含 `&&` 后 rm、`sudo rm`、`xargs rm`、`os.remove`、`shutil.rmtree`、`Remove-Item`）＋设计放行 13 例 | 26＋2 附检 | **不符 8**（误伤 7 ＋ 漏拦 `shutil.rmtree`） | 不符 0 |
| 旁通路数 | 账可写／账不可写／根不存在／无旁通／非 JSON 输入 | 6 | **不符 5** | 不符 0 |
| 旁通作用域 | 绑定式覆盖与不覆盖／全局式／无关写不入账／无旁通 | 10 | **不符 8** | 不符 0 |

误伤样本（v0.1.0 会拦、v0.1.1 放行）：命令里出现 `cbb-guard`、`third`、`record`、`platform`、`warm`、`standard`、`confirm` 这类以 `rm`/`rd`/`del` 收尾的普通词，且同条命令提到 `迷深实战-*`。
夹具脚本本身不在本仓（写在会话侧 `~/.qoder/tmp/cbb-fix/`），需要复跑请照上表口径重造。

## 版本

- **0.1.1（2026-09-24）**：G-1 清理动词改独立词匹配＋补真删除 API（误伤 7／漏拦 1 → 0）；G-2 旁通留痕写不进即拒绝放行（原为静默放行且零留痕）；G-3 旁通改为**先判越权再谈放行**＋绑定式作用域＋无关写不灌账；拦截回执加命中面；根路径不存在时显式警告。规则范围（拦 A/拦 B 的清单）未扩大也未缩小。
